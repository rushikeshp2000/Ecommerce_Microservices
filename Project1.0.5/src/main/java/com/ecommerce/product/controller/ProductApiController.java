...
public class ProductApiController implements ProductApi {
	...
	public ResponseEntity<ProductResponse> getAllCategoryProducts(
			...
			@ApiParam(value = "", required = true) @PathVariable("category") String category,

			@ApiParam(value = "") @Valid @RequestParam(value = "productname", required = false) String productname,
		...
		List<Product> viewAllProductsOfCategory = service.viewAllProductsOfCategory(category, offset, limit);
		...
		productRes.setPageDate(pageData);
		return new ResponseEntity<ProductResponse>(productRes, HttpStatus.OK);
	}

	public ResponseEntity<Void> deleteProduct(

			@ApiParam(value = "The name that needs to be deleted", required = true) @PathVariable("ProductName") String productName,
	...
	public ResponseEntity<ProductResponse> allProductsBySellerId(
			...
			@ApiParam(value = "", required = true) @PathVariable("sellerID") String sellerID) {
		List<Product> bysellerId = service.getBysellerId(sellerID);
	...
	public ResponseEntity<ProductResponse> getAllSellerProducts(
			...
			@ApiParam(value = "", defaultValue = "15") @Valid @RequestParam(value = "limit", required = false, defaultValue = "15") Integer limit) {
		List<Product> viewAllProductsFromSeller = service.viewAllProductsFromSeller(category, sellerID, offset, limit);
		ProductResponse productRes = new ProductResponse();
	...
	public ResponseEntity<ProductResponse> getStatusProducts(
			...
			@ApiParam(value = "", required = true) @PathVariable("statusId") String statusId) {

		List<Product> bySellerIdAndStatus = service.getBySellerIdAndStatus(sellerID, statusId);
		ProductResponse resultObject = new ProductResponse();
	...
	public ResponseEntity<ProductResponse> productStatusSellerIDStatusIdCategoryPut(
		...
		List<Product> bySellerIdAndStatusCategory = service.getBySellerIdAndStatusCategory(sellerID, statusId,
				category);
		ProductResponse resultObject = new ProductResponse();
	...
	public ResponseEntity<ProductResponse> updateSingleProducts(
			...
			@ApiParam(value = "") @RequestPart(value = "status", required = false) String status,

			@ApiParam(value = "") @Valid @RequestPart(value = "", required = false) List<MultipartFile> images) {

		Product updateAProductOfSeller = null;
		try {
			updateAProductOfSeller = service.UpdateAProductOfSeller(name, Double.parseDouble(price), details,
					Integer.parseInt(quantity), sellerNumber, category, Integer.parseInt(discount), status, images);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		resultObject.setStatusCode(HttpStatus.ACCEPTED.value());
		resultObject.setStatusMessage("A single product according to the ProductName");
		System.out.println(updateAProductOfSeller + "here");
		return new ResponseEntity<ProductResponse>(resultObject, HttpStatus.OK);
	}
	...
	public ResponseEntity<ProductResponse> updateQuantityProduct(
		...
		Product updateQuantity = service.updateQuantity(productName, Integer.parseInt(quantity));
		ProductResponse resultObject = new ProductResponse();
		resultObject.addResultItem(updateQuantity);
		resultObject.setStatusCode(HttpStatus.ACCEPTED.value());
		resultObject.setStatusMessage("A single product according to the ProductName");
		return new ResponseEntity<ProductResponse>(resultObject, HttpStatus.OK);
	}
